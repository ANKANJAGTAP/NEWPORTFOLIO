...
const STATES = {
  ...
  // EXPERIENCE: aligned with the reference pattern (desktop: moved right; mobile: compact)
  experience: {
    desktop: {
      scale: { x: 0.34, y: 0.34, z: 0.34 },
      position: { x: 600, y: -350, z: 0 }, // right-side placement for desktop
      rotation: {
        x: 0,
        y: Math.PI / 8,
        z: 0,
    ...
    mobile: {
      scale: { x: 0.18, y: 0.18, z: 0.18 },
      position: { x: 0, y: -20, z: 0 }, // centered for mobile
      rotation: {
        x: 0,
        y: Math.PI / 6,
        z: 0,
  ...
  // EDUCATION: same responsive pattern as experience (desktop further right to sit in right-most column)
  education: {
    desktop: {
      scale: { x: 0.34, y: 0.34, z: 0.34 },
...
const AnimatedBackground = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Behavior based on activeSection (include experience & education handling)
      if (activeSection === "hero") {
        ...
        // pause rotation when in experience/education (we want it to settle into place)
        rotateKeyboard.restart();
        teardownKeyboard.pause();
      } else {
        rotateKeyboard.pause();
  ...
  const handleGsapAnimations = () => {
    ...
    // EXPERIENCE trigger (NEW) - will move keyboard into experience area
    gsap.timeline({
      scrollTrigger: {
        trigger: "#experience",
        start: "top 60%",
        ...
        onLeave: () => {
          // user scrolls forward past experience -> set to projects (projects trigger will also run)
          setActiveSection("projects");
        },
      },
    });

    // EDUCATION trigger (NEW) - moves keyboard to the right-most education area
    gsap.timeline({
      scrollTrigger: {
        trigger: "#education",
        start: "top 60%",
        ...
        onLeaveBack: () => {
          // user scrolled back to experience
          setActiveSection("projects");
          gsap.to(kbd.scale, {
        ...
        onLeave: () => {
          // user scrolls forward past education -> set to projects (or next)
          setActiveSection("contact");
        },
      },
    });

    // PROJECTS trigger (existing)
    gsap.timeline({
      scrollTrigger: {
        trigger: "#projects",
        start: "top 70%",
